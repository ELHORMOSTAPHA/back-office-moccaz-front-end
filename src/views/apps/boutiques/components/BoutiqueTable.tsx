...
import type { BoutiqueBulkStatusFilters, BoutiqueType } from '@/interface/gloable'
import DataTable from '@/components/table/DataTable'
import TablePagination from '@/components/table/TablePagination'
import TableSkeleton from '@/components/TableSkeleton'
import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query'
import { bulkUpdateBoutiqueStatus, loadBoutiques, updateBoutique } from '@/services/boutiques'
const columnHelper = createColumnHelper<BoutiqueType>()
...
type BoutiqueSortId = (typeof BOUTIQUE_SORT_IDS)[number]

const COLUMN_KEY = 'moccaz.boutique-table.column-visibility'
const FILTER_KEY = 'moccaz.boutique-table.filter-visibility'

type BoutiqueFilterVisibility = {
    global: boolean
    date: boolean
    status: boolean
    city: boolean
...
const DEFAULT_FILTER_VISIBILITY: BoutiqueFilterVisibility = {
    global: true,
    date: false,
    status: false,
    city: false,
...
const FILTER_LABELS: Record<keyof BoutiqueFilterVisibility, string> = {
    global: 'Recherche',
...
const OPTIONAL_FILTER_ORDER = ['date', 'status', 'city', 'par_page'] as const satisfies readonly (
    keyof BoutiqueFilterVisibility
)[]
...
const SKELETON_COLUMNS: SkeletonCol[] = [
    ...
    { id: 'user_id', width: '220px', type: 'text' },
    { id: 'address', width: '180px', type: 'text' },
    { id: 'city', width: '140px', type: 'text' },
    { id: 'pack_id', width: '160px', type: 'text' },
    { id: 'created_by', width: '88px', type: 'text' },
    { id: 'status', width: '160px', type: 'text' },
...
const BoutiqueTableComponent = () => {
    ...
    const [filterVisibility, setFilterVisibility] = useState<BoutiqueFilterVisibility>(() => ({
        ...DEFAULT_FILTER_VISIBILITY,
    }))

    const sortByRaw = sorting[0]?.id
    const sort_by: BoutiqueSortId = BOUTIQUE_SORT_IDS.includes(sortByRaw as BoutiqueSortId)
        ? (sortByRaw as BoutiqueSortId)
        : 'created_at'
    ...
    const { data: boutiqueResponse, isLoading, isError, error } = useQuery({
        queryKey: ['boutiques', apiParams],
        queryFn: () => loadBoutiques(apiParams),
        staleTime: 30_000,
    })

    const statusMutation = useMutation({
        mutationFn: ({ id, status }: { id: number; status: number }) => updateBoutique(id, { status }),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['boutiques'] })
        },
    })

    const bulkListFilters = useMemo((): BoutiqueBulkStatusFilters | undefined => {
        const f: BoutiqueBulkStatusFilters = {}
    ...
    const bulkStatusMutation = useMutation({
        mutationFn: bulkUpdateBoutiqueStatus,
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['boutiques'] })
            setSelectionState({
                selectAll: false,
    ...
    const data = useMemo(() => boutiqueResponse?.data ?? [], [boutiqueResponse])
    const totalItems = boutiqueResponse?.pagination?.total ?? 0
    const start = boutiqueResponse?.pagination?.from ?? 0
    const end = boutiqueResponse?.pagination?.to ?? 0
    ...
    const persistFilters = (next: BoutiqueFilterVisibility) => {
        try {
    ...
    useEffect(() => {
        try {
            ...
            if (!raw) return
            const p = JSON.parse(raw) as Partial<BoutiqueFilterVisibility>
    ...
    const toggleOptionalFilter = (fid: keyof BoutiqueFilterVisibility) => {
    ...
    const runBulkStatusUpdate = (status: 0 | 1 | 2 | 3) => {
    ...
    const columns: ColumnDef<BoutiqueType, any>[] = [
        {
            id: 'select',
            maxSize: 45,
            size: 45,
            ...
            cell: ({ row }: { row: TableRow<BoutiqueType> }) => (
                <input
                    type="checkbox"
                    className="form-check-input form-check-input-light fs-14"
        ...
        columnHelper.accessor('user_id', {
            ...
            cell: ({ row }) => {
                ...
                const fullName = `${client.nom ?? ''} ${client.prenom ?? ''}`.trim() || `#${row.original.user_id}`
                return (
                    <div className="d-flex align-items-center gap-2">
                        ...
                        <div className="min-w-0">
                            <h6 className="mb-0 text-truncate" style={{ maxWidth: 220 }} title={fullName}>
                                {fullName}
                            </h6>
                            <p
                                className="text-muted fs-xs mb-0 text-truncate"
                                style={{ maxWidth: 220 }}
                                title={client.email || client.telephone || ''}
                            >
        ...
        columnHelper.accessor('pack_id', {
            ...
            cell: ({ row }) => (
                <span className="text-nowrap">
                    {row.original.boutique_pack?.name?.trim() ||
                        (row.original.pack_id != null ? `#${row.original.pack_id}` : '—')}
                </span>
            ),
        }),
        columnHelper.accessor('created_by', {
            id: 'created_by',
            header: 'Créé par',
            cell: ({ row }) => <span className="text-nowrap">{row.original.created_by}</span>,
        }),
        columnHelper.accessor('status', {
            ...
            cell: ({ row }) => {
                ...
                return (
                    <div style={{ minWidth: 168 }} className="boutique-table-status-select">
                        <Select
    ...
    const table = useReactTable({
        ...
        manualSorting: true,
        pageCount: boutiqueResponse?.pagination?.last_page ?? 0,
    })
                    ...
                    <div className="d-flex align-items-center justify-content-between gap-2 flex-wrap mb-2">
                        ...
                        <Dropdown align="end" autoClose="outside">
                            <Dropdown.Toggle
                                ...
                                id="boutique-filter-visibility-toggle"
                            ...
                            <Col xs={12} md={8} lg={6}>
                                ...
                                <div className="app-search">
                                    <input
                                        ...
                                        placeholder="Nom de la boutique, email, téléphone…"
                                        value={keyword}
                                        onChange={(e) => {
            ...
            <Card>
                    ...
                    <div className="d-flex align-items-center justify-content-between gap-2 flex-wrap w-100">
                                ...
                                <>
                                    ...
                                    <Dropdown>
                                        <Dropdown.Toggle
                                            ...
                                            size="sm"
                                            id="boutique-bulk-status-dropdown"
                                        ...
                                        <Dropdown.Menu>
                                            ...
                                            <Dropdown.Item
                                                onClick={() => runBulkStatusUpdate(2)}
                                                disabled={bulkStatusMutation.isPending}
                                            >
                                                Validé
                                            </Dropdown.Item>
                                            <Dropdown.Item
                                                onClick={() => runBulkStatusUpdate(3)}
                                                disabled={bulkStatusMutation.isPending}
                                            >
                                                Rejeté
                                            </Dropdown.Item>
                        ...
                        <Dropdown align="end" autoClose="outside">
                            <Dropdown.Toggle
                                ...
                                size="sm"
                                className="btn-icon rounded-circle"
                                id="boutique-table-columns-toggle"
                ...
                {isLoading ? (
                    ...
                    <div className="alert alert-danger m-3">
                        Erreur lors du chargement des boutiques: {error?.message}
                    </div>
                ) : (
                    <DataTable<BoutiqueType> table={table} emptyMessage="Aucune boutique" />
                )}
                ...
                {data.length > 0 && !isLoading && (
                    <CardFooter className="border-0">
                        <TablePagination
                            ...
                            canNextPage={
                                pagination.pageIndex < (boutiqueResponse?.pagination?.last_page ?? 1) - 1
                            }
                        />
                    </CardFooter>
                )}
...
export default BoutiqueTableComponent
